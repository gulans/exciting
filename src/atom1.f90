...
!
!
Subroutine atom1 (ptnucl, zn, nst, n, l, k, occ, xctype, xcgrad, nr, &
&  r, eval, rho, vr, rwf, mtnr, dirac_eq,is)
! !USES:
      Use modxcifc
      use modinput
      use modinteg
...
!$OMP& SHARED(nst,n,l,k,nr,r,vr,eval,rwf,dirac_eq,sloppy,is)
!$OMP DO
         Do ist = 1, nst
            Call rdirac1 (0, n(ist), l(ist), k(ist), nr, r, vr, &
           &  eval(ist), rwf(:, 1, ist), rwf(:, 2, ist), dirac_eq, sloppy, is)
         End Do
!$OMP END DO
!$OMP END PARALLEL
