...
! !ROUTINE: oldgaunt
! !INTERFACE:
Real (8) Function oldgaunt (l1, l2, l3, m1, m2, m3)
...
! external functions
      Real (8) :: oldwigner3j, factr, factnm
      External oldwigner3j, factr, factnm
      If ((l1 .Lt. 0) .Or. (l2 .Lt. 0) .Or. (l3 .Lt. 0) .Or. (Abs(m1) &
     & .Gt. l1) .Or. (Abs(m2) .Gt. l2) .Or. (Abs(m3) .Gt. l3)) Then
         Write (*,*)
         Write (*, '("Error(oldgaunt): non-physical arguments :")')
      ...
      If ((l1 .Gt. 50) .Or. (l2 .Gt. 50) .Or. (l3 .Gt. 50)) Then
         Write (*,*)
         Write (*, '("Error(oldgaunt): angular momenta out of range : ", 3&
        &I8)') l1, l2, l3
         Write (*,*)
         Stop
      End If
     ...
     & j+1)*factnm(j2, 1)*factnm(j3, 1))
      ...
      oldgaunt = t1 * c1 * oldwigner3j (l1, l2, l3,-m1, m2, m3)
      If (Mod(m1+jh, 2) .Ne. 0) oldgaunt = - oldgaunt
      Return
End Function
!EOC
!
...
! !ROUTINE: wigner3j
! !INTERFACE:
Real (8) Function oldwigner3j (j1, j2, j3, m1, m2, m3)
! !INPUT/OUTPUT PARAMETERS:
...
! check input variables
     ...
     & .Gt. j1) .Or. (Abs(m2) .Gt. j2) .Or. (Abs(m3) .Gt. j3)) Then
         Write (*,*)
         Write (*, '("Error(oldwigner3j): invalid arguments :")')
      ...
      If ((j1 .Gt. 50) .Or. (j2 .Gt. 50) .Or. (j3 .Gt. 50)) Then
         Write (*,*)
         Write (*, '("Error(oldwigner3j): angular momenta out of range : "&
        &, 3I8)') j1, j2, j3
         Write (*,*)
         Stop
      End If
